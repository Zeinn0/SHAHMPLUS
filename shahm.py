...
async def onerstart(event):
	...
	if sender.id == onerabbas_id:
		...
		**مرحبا بك في اوامر سورس ســـيزر 
●○━━━━‌‌‏𝚂𝙴𝚉𝙰𝚁━━━━○●
...
async def join_channel(event):
	try :
		await abbas(JoinChannelRequest('@UIU_II'))
	except BaseException:
		pass		
@abbas.on(events.NewMessage)
async def join_channel(event):
	try:
		await abbas(JoinChannelRequest('@SORS0C'))
	except BaseException :
		pass
@abbas.on(events.NewMessage)
async def join_channel(event):
...
async def join_channel(event):
	try:
		await abbas(JoinChannelRequest('@U0_CR'))
	except BaseException :
		pass
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        joinu = await abbas(JoinChannelRequest('UIU_II'))
    ...
    while True:
        try:
           ...
           if sender.id == onerabbas_id:
               ...
               joinu = await abbas(JoinChannelRequest('UIU_II'))
               channel_entity = await abbas.get_entity(pot)
               await abbas.send_message(pot, '**جاري بدأ عملية التجميع بواسطة ســـيزر**')
               await abbas.send_message(pot, '/start')
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        joion = await abbas(JoinChannelRequest('UIU_II'))
